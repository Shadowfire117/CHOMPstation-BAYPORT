		...
		if(BP_HEAD, BP_MOUTH, BP_EYES)
			...
			switch(attack_damage)
				if(1 to 2)
					...
					target.bloodstr.add_reagent(/datum/reagent/numbenzyme,attack_damage) //Have to add this here, otherwise the swtich fails.
				if(3 to 4)
					...
					target.bloodstr.add_reagent(/datum/reagent/numbenzyme,attack_damage)
				if(5)
