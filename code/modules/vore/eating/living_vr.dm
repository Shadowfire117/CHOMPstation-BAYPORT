...
/mob/living/proc/vore_attackby(obj/item/I,mob/user)
	...
	if(istype(I,/obj/item/grab))
		...
		if((G.state >= NORM_AGGRESSIVE) && (isliving(user) && !issilicon(G.affecting)))
			...
			else if((src == G.affecting) && (attacker.a_intent == I_GRAB) && (attacker.zone_sel.selecting == BP_CHEST) && (is_vore_predator(G.affecting)))
...
/mob/living/proc/eat_trash()
	...
	if(is_type_in_list(I,edible_trash))
		...
		else if(istype(I,/obj/item/device/flashlight/flare/glowstick))
