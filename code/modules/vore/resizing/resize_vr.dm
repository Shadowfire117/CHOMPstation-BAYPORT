		...
		if(I_DISARM)
			...
			if((get_effective_size() - tmob.get_effective_size()) >= 0.75)
				...
				if(MOVING_QUICKLY(src))
					...
					log_attack(src,tmob,"Pinned underfoot (run, no halloss)")
				...
				//Walking on I_DISARM
					...
					log_attack(src,tmob,"Pinned underfoot (walk, about [damage] halloss)")
		...
		if(I_GRAB)
			...
			if((get_effective_size() - tmob.get_effective_size()) >= 0.50)
				...
				if(!H || (!isTaurTail(H.tail_style) && H.shoes))
					...
					log_attack(src,tmob,"Grabbed underfoot (nontaur, shoes)")
				...
				//Human, not a taur, but not wearing shoes = yes grab
					...
					log_attack(src,tmob,"Grabbed underfoot (nontaur, no shoes)")
				...
				//Human, taur, shoes = no grab, special message
					...
					log_attack(src,tmob,"Grabbed underfoot (taur, shoes)")
				...
				//Human, taur, no shoes = yes grab, special message
					...
					equip_to_slot_if_possible(tmob.get_scooped(H), slot_shoes, 0, 1)
					log_attack(src,tmob,"Grabbed underfoot (taur, no shoes)")
