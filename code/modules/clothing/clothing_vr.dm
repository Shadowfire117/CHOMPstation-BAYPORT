...
/obj/item/clothing/suit/equipped(var/mob/user, var/slot)
	...
	if(!taurized && slot == slot_wear_suit && ishuman(user))
		...
		if(isTaurTail(H.tail_style))
			...
			if(taurtail.suit_sprites && (get_icon_state(slot_wear_suit_str) in icon_states(taurtail.suit_sprites)))
...
/obj/item/clothing/suit/get_mob_overlay(var/body_type,var/slot_name,var/inhands,var/default_icon,var/default_layer = 0)
	var/image/standing = ..()
