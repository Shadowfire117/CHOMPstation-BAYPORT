...
//Regular jobs
	...
	// call dibs on IC messages (prays, emergency comms, faxes)
		...
		if(isliving(M))
			...
			health_description += "<BR>Oxy: [L.getOxyLoss()] - Tox: [L.getToxLoss()] - Fire: [L.getFireLoss()] - Brute: [L.getBruteLoss()] - Clone: [L.getCloneLoss()] - Brain: [L.getBrainLoss()] - Digest: [L.getDigestLoss()]" //Chompstation ADD - Adding digest damtype to the VV.
		else
