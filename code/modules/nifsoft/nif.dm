	...
	if(ishuman(newloc))
		...
		if(!quick_implant(H))
			...
			spawn(0)
				qdel(src)
			return FALSE
/*		else  // Commlinks temporarily disabled - Jon
...
/obj/item/device/nif/proc/implant(var/mob/living/carbon/human/H)
	var/obj/item/organ/brain = H.internal_organs_by_name[BP_BRAIN]
	if(istype(brain))
		should_be_in = brain.parent_organ

	if(istype(H) && !H.nif && H.species && (loc == H.get_organ(should_be_in)))
		if(!bioadap && (H.species.species_flags & SPECIES_FLAG_NO_SCAN)) //NO_SCAN is the default 'too complicated' flag
			return FALSE
...
/obj/item/device/nif/proc/handle_install()
	...
	if(human.client)
		human.overlay_fullscreen("scanlines",/obj/screen/fullscreen/scanline)
		human.overlay_fullscreen("whitenoise",/obj/screen/fullscreen/noise) // Camera static.
...
/*			if(comm) // Commlink has been removed :V - Jon
...
/obj/item/device/nif/attack(mob/living/M, mob/living/user, var/target_zone)
	...
	if(istype(T.species,/datum/species/shapeshifter/promethean) && target_zone == BP_CHEST)
		if(T.w_uniform || T.wear_suit)
			to_chat(user,"<span class='warning'>Remove any clothing they have on, as it might interfere!</span>")
			return
		var/obj/item/organ/external/eo = T.get_organ(BP_CHEST)
		if(!T)
		...
		if(do_mob(user = user, target = T, time = 200, target_zone = BP_CHEST))
			user.unEquip(src)
			forceMove(eo)
			eo.implants |= src
			implant(T)
