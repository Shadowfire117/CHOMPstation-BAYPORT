...
/obj/item/organ/external/head/removed()
	...
	for(var/M in markings)
		...
		mark_s.Blend(markings[M]["color"], mark_style.color_blend_mode) // MITHRAstation edit
...
/obj/item/organ/external/on_update_icon(var/regenerate = 0)
	...
	if(body_hair && islist(h_col) && h_col.len >= 3)
		...
		if(!limb_icon_cache[cache_key])
			...
			I.Blend(rgb(h_col[1],h_col[2],h_col[3]), ICON_MULTIPLY) //Mithrastation edit
	...
	if(model)
		...
		/*
		// Mithrastation edit to enable markings on synths
